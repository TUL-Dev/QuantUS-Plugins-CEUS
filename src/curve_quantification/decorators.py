...
def dependencies(*dep_names: List[str]):
    """
    A decorator to specify the dependencies for a function.

    Args:
        dep_names (list): List of dependency names.

    Returns:
        function: The decorated function with the specified keyword arguments attached as metadata.
    """
    def decorator(func):
        func.dependencies = dep_names
        return func
    return decorator
